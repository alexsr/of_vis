...
void main() {
    ...
    vec4 temp_tangent_p1 = line_p2 - line_p0;
    vec4 temp_tangent_p2 = line_p3 - line_p1;
    vec4 sideways_vector_p1 = vec4(normalize(cross(line_p1.xyz, temp_tangent_p1.xyz)), 0.0f);
    vec4 sideways_vector_p2 = vec4(normalize(cross(line_p2.xyz, temp_tangent_p2.xyz)), 0.0f);

    tangent = temp_tangent_p1;
    pass_uv = vec2(pass_uv_x[1], 0);
    ...
    float next_uv_x = pass_uv_x[2];
    if (next_uv_x < pass_uv_x[1]) {
        next_uv_x = pass_uv_x[1];
    }
    pass_uv = vec2(next_uv_x, 0);
