...
int main() {
    ...
    tostf::V_F_shader visualization_shader("../../shader/visualization/boundary_volume_ray_casting.vert",
                                           "../../shader/visualization/boundary_volume_ray_casting.frag");
    ...
    tostf::foam::Volume_grid test_grid(an_bb, 0.001f);
    ...
    std::vector<float> field_data_for_tex(test_grid.total_cell_count);
    const auto load_field = [&field_data_for_tex, &player, &test_grid]() {
...
#pragma omp parallel for reduction(+:field_data,weight_sum) schedule(dynamic,1)
                ...
                field_data_for_tex.at(cell_index) = -FLT_MAX;
            }
        }
    };
    ...
    visualization_shader.update_uniform("max_steps", max_steps);
    visualization_shader.update_uniform("step_size", step_size);
    ...
    while (!window.should_close()) {
        ...
        if (ImGui::Begin("Volume Ray Casting controls")) {
            ImGui::DragFloat("Step size", &step_size, 0.000001f, 0.000001f, 1000.0f, "%.5f");
            ImGui::DragInt("Max step count", &max_steps, 1, 0, 10000);
            visualization_shader.update_uniform("step_size", step_size);
            visualization_shader.update_uniform("max_steps", max_steps);
        }
        ImGui::End();
        ...
        visualization_shader.update_uniform("camera_pos", cam.get_position());
